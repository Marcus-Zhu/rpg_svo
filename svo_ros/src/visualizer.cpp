...
void Visualizer::displayKeyframeWithMps(const FramePtr &frame, int ts)
{
	...
	vk::output_helper::publishFrameMarker(
		...
		T_world_cam.translation(), "kfs", ros::Time::now(), frame->id_ * 10, 0, 0.045);
	...
	for(Features::iterator it = frame->fts_.begin(); it != frame->fts_.end(); ++it)
	{
		...
		//inserted code for level map
		Vector3d pos = T_world_from_vision_ * (*it)->point->pos_;
		level_map_.update(pos);
		level_map_.publish();
		//end

		vk::output_helper::publishPointMarker(
			pub_points_, T_world_from_vision_ * (*it)->point->pos_, "pts",
			ros::Time::now(), (*it)->point->id_, 0, 0.015, Vector3d(0.0, 0.0, 1.0),
